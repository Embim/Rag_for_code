...
class SearchRequest(BaseModel):
    ...
    strategy: Optional[Literal[
        "semantic", "ui_to_database", "database_to_ui",
        # Deprecated aliases (mapped to semantic in router):
        "bm25", "hybrid", "multi_hop", "impact_analysis", "pattern_search",
    ]] = Field(
        default="semantic",
        description=(
            "Search strategy: 'semantic' (default), 'ui_to_database', "
            "'database_to_ui'. Old values (bm25/hybrid/multi_hop/...) are "
            "accepted for backward compatibility but mapped to 'semantic'."
        ),
    )
...
class AskRequest(BaseModel):
    ...
    max_iterations: Optional[int] = Field(
        ...
        le=20,
        description=(
            "Max RAG-loop iterations (rewrite + retrieve + check). "
            "Default 3 — обычно score достигает threshold на 1-2-й итерации. "
            "Поднимай только для сложных/адверсариальных запросов."
        ),
    )
    timeout: Optional[int] = Field(
        ...
        ge=10,
        le=1800,
        description="Timeout in seconds (max 30 min for long RAG iterations)"
    )
