...
class RuntimeCacheConfig:
    """
    Runtime cache configuration (in-memory LRU + optional Redis).

    Не путать с :class:`src.infra.config.cache.CacheConfig` — это статический
    конфиг (BaseConfig), а здесь — runtime-параметры самого кеша.
    """
    enabled: bool = True
...
# Backward-compat alias — старый импорт `from src.infra.cache import CacheConfig`
# и `from src.agents.cache import CacheConfig` продолжает работать.
